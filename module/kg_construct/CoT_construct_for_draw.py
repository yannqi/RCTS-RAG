...
async def Self_CoT_extract(
    ...
    async def _process_single_content(prompt_dict):
        ...
        for index in range(CoT_loop_times):
            ...
            temp_answer_list.append(pred_answer_list)
            ...
            if pred_judge_content[index] == len(pred_answer_list):
                break  # TODO 出现满分答案，直接跳出
        previous_answer_list.append(temp_answer_list)
        previous_CoT_list.append(pred_CoT_list)
        ...
        if already_processed % 50 == 0 and already_processed > 0:  # save preds every 10 samples
            ...
            preds_df = pd.DataFrame({'question_id': question_id_list, 'pred_CoT': best_pred_CoT_list, 'pred_judge': pred_judge_list, 'previous_CoT': previous_CoT_list, 'previous_answer': previous_answer_list})
